module reward_distribution::RewardDistribution {
    ...
    public entry fun initialize_activity(user: &signer, ctx: &mut TxContext) {
        ...
        move_to(user, UserActivity { id, completed_modules: 0, discussions: 0, content_created: 0 });
        emit<InitializeActivityEvent>(InitializeActivityEvent { user: signer::address_of(user) });
    }

    // Record user activity.
    public entry fun record_activity(user: &signer, modules: u64, discussions: u64, content: u64, ctx: &mut TxContext) {
        ...
        activity.content_created = activity.content_created + content;
        emit<RecordActivityEvent>(RecordActivityEvent { user: signer::address_of(user), modules, discussions, content });
    }

    // Distribute rewards based on recorded activity.
    public entry fun distribute_rewards(user: &signer, ctx: &mut TxContext) {
        let activity = borrow_global_mut<UserActivity>(signer::address_of(user));
        let total_rewards = activity.completed_modules * 10 + activity.discussions * 5 + activity.content_created * 20;
    ...
    public fun get_activity(user: address): (u64, u64, u64) {
        let activity = borrow_global<UserActivity>(user);
        (activity.completed_modules, activity.discussions, activity.content_created)
    }
}
